...
const buildResources = () => {
  ...
  Promise.all(promises).then(
    (res) => {
      ...
      console.log('+++++ ', process.env.sunbird_azure_account_name); // TODO: log!
      console.log('+++++ ', process.env.sunbird_cloud_storage_provider); // TODO: log!
    },
    (error) => {
      ...
      (allContents) => {
        ...
        _.forEach(resObj, (data, lang) => {
          try {
            ...
            console.log('⏳ buildBundle :: Updating resource bundles for language', lang);
          } catch (e) {
  ...
  return readDirPr.then((filenames) => {
    return Promise.all(
      filenames.map((filename) => {
        return new Promise((resolve, reject) => {
          console.log('✅ Reading files [ ' + dirname + filename + ' ]');
